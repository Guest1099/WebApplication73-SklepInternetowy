...
namespace Data
{
    public class ApplicationDbContext : IdentityDbContext<ApplicationUser, ApplicationRole, string>
    {
        ...
        protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
        {
            ...
            {
                optionsBuilder.UseSqlServer(@"Data Source=(localdb)\MSSQLLocalDB;Initial Catalog=WebApplication73-Mvc-SklepInternetowy;Integrated Security=True;Connect Timeout=30;Encrypt=False;Trust Server Certificate=False;Application Intent=ReadWrite;Multi Subnet Failover=False");
            }
        }
        ...
        private void CreateInitialData(ModelBuilder builder)
        {
            ...
            */
            builder.Entity<ApplicationUser>()
               .HasMany(h => h.Orders).WithOne(w => w.User).HasForeignKey(f => f.UserId).OnDelete(DeleteBehavior.ClientSetNull);

            builder.Entity<ApplicationUser>()
               .HasMany(h => h.Favourites).WithOne(w => w.User).HasForeignKey(f => f.UserId).OnDelete(DeleteBehavior.ClientSetNull);

            builder.Entity<ApplicationUser>()
                .HasMany(h => h.Likes).WithOne(w => w.User).HasForeignKey(f => f.UserId).OnDelete(DeleteBehavior.ClientNoAction);
            ...
            builder.Entity<Category>()
                .HasMany(h => h.Products).WithOne(w => w.Category).HasForeignKey(f => f.CategoryId).OnDelete(DeleteBehavior.ClientSetNull);
            ...
            builder.Entity<Subcategory>()
                .HasMany(h => h.Products).WithOne(w => w.Subcategory).HasForeignKey(f => f.SubcategoryId).OnDelete(DeleteBehavior.ClientSetNull);
            ...
            builder.Entity<DaneOsobowe>()
               .HasMany(h => h.Users).WithOne(w => w.DaneOsobowe).HasForeignKey(f => f.DaneOsoboweId).OnDelete(DeleteBehavior.ClientSetNull);

            builder.Entity<DaneOsobowe>()
               .HasMany(h => h.Clients).WithOne(w => w.DaneOsobowe).HasForeignKey(f => f.DaneOsoboweId).OnDelete(DeleteBehavior.ClientSetNull);

            builder.Entity<DaneOsobowe>()
               .HasMany(h => h.PhotosDaneOsobowe).WithOne(w => w.DaneOsobowe).HasForeignKey(f => f.DaneOsoboweId).OnDelete(DeleteBehavior.ClientSetNull);
            ...
            builder.Entity<Product>()
                .HasMany(h => h.Favourites).WithOne(w => w.Product).HasForeignKey(f => f.ProductId).OnDelete(DeleteBehavior.ClientSetNull);

            builder.Entity<Product>()
                .HasMany(h => h.Likes).WithOne(w => w.Product).HasForeignKey(f => f.ProductId).OnDelete(DeleteBehavior.ClientSetNull);
            ...
            List<string> daneOsoboweId = new List<string>();

            // dodanie danych osobowych
            for (var i = 0; i < 10; i++)
            {
                DaneOsobowe daneOsobowe = new DaneOsobowe()
                {
                    ...
                    KodPocztowy = "21-222",
                    Wojewodztwo = _dataAutogenerator.Nazwisko(),
                    ...
                    Firma_Wojewodztwo = _dataAutogenerator.Nazwisko(),
                ...
                if (i == 0)
                {
                    ...
                    IdentityUserRole<string> applicationUserRoleAdmin = new IdentityUserRole<string>()
                    {
                        UserId = administratorUser.Id,
                        RoleId = adminRole.Id
                    };
                    builder.Entity<IdentityUserRole<string>>().HasData(applicationUserRoleAdmin);
                }
                ...
                if (i == 1)
                {
                    var aaaUser = new ApplicationUser()
                    {
                        ...
                        DaneOsoboweId = daneOsobowe.DaneOsoboweId,
                        DataOstatniegoZalogowania = DateTime.Now.ToString(),
                        NormalizedUserName = "aaa@aaa.pl".ToUpper(),
                        NormalizedEmail = "aaa@aaa.pl".ToUpper(),
                        SecurityStamp = Guid.NewGuid().ToString(),
                        ConcurrencyStamp = Guid.NewGuid().ToString(),
                        EmailConfirmed = true,
                    };
                    aaaUser.PasswordHash = passwordHasher.HashPassword(aaaUser, "SDG%$@5423sdgagSDert");
                    builder.Entity<ApplicationUser>().HasData(aaaUser);
                    IdentityUserRole<string> applicationUserRoleAaaUser = new IdentityUserRole<string>()
                    {
                        UserId = aaaUser.Id,
                        RoleId = userRole.Id
                    };
                    builder.Entity<IdentityUserRole<string>>().HasData(applicationUserRoleAaaUser);
                }
                ...
                if (i == 2)
                {
                    var bbbUser = new ApplicationUser()
                    {
                        ...
                        DaneOsoboweId = daneOsobowe.DaneOsoboweId,
                        DataOstatniegoZalogowania = DateTime.Now.ToString(),
                        NormalizedUserName = "bbb@bbb.pl".ToUpper(),
                        NormalizedEmail = "bbb@bbb.pl".ToUpper(),
                        SecurityStamp = Guid.NewGuid().ToString(),
                        ConcurrencyStamp = Guid.NewGuid().ToString(),
                        EmailConfirmed = true,
                    };
                    bbbUser.PasswordHash = passwordHasher.HashPassword(bbbUser, "SDG%$@5423sdgagSDert");
                    builder.Entity<ApplicationUser>().HasData(bbbUser);
                    IdentityUserRole<string> applicationUserRoleBbbUser = new IdentityUserRole<string>()
                    {
                        UserId = bbbUser.Id,
                        RoleId = userRole.Id
                    };
                    builder.Entity<IdentityUserRole<string>>().HasData(applicationUserRoleBbbUser);
                }
                ...
                if (i == 3)
                {
                    var cccUser = new ApplicationUser()
                    {
                        ...
                        DaneOsoboweId = daneOsobowe.DaneOsoboweId,
                        DataOstatniegoZalogowania = DateTime.Now.ToString(),
                        NormalizedUserName = "ccc@ccc.pl".ToUpper(),
                        NormalizedEmail = "ccc@ccc.pl".ToUpper(),
                        SecurityStamp = Guid.NewGuid().ToString(),
                        ConcurrencyStamp = Guid.NewGuid().ToString(),
                        EmailConfirmed = true,
                    };
                    cccUser.PasswordHash = passwordHasher.HashPassword(cccUser, "SDG%$@5423sdgagSDert");
                    builder.Entity<ApplicationUser>().HasData(cccUser);
                    IdentityUserRole<string> applicationUserRoleCccUser = new IdentityUserRole<string>()
                    {
                        UserId = cccUser.Id,
                        RoleId = userRole.Id
                    };
                    builder.Entity<IdentityUserRole<string>>().HasData(applicationUserRoleCccUser);
                }




                // id danych osobowych dodawane są dopiero od numeru 4, ponieważ wcześniejsze należą do użytkowników systemu
                if (i > 3)
                    daneOsoboweId.Add(daneOsobowe.DaneOsoboweId);

            }


            // dodanie klientów do bazy
            for (var i = 0; i < daneOsoboweId.Count; i++)
            {
                Client client = new Client()
                {
                    ClientId = Guid.NewGuid().ToString(),
                    DaneOsoboweId = daneOsoboweId[i],
                };
